...
void insertEdge(LIST L, int* edge) {
    NODETYPE newNode1 = (NODETYPE)malloc(sizeof(struct node));
    NODETYPE newNode2 = (NODETYPE)malloc(sizeof(struct node));
    if(newNode1 && newNode1) {
      newNode1->data = edge[1];
      newNode1->next = L[edge[0]];
      L[edge[0]] = newNode1;

      newNode2->data = edge[0];
      newNode2->next = L[edge[1]];
      L[edge[1]] = newNode2;
    } else {
